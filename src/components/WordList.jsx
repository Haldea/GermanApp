...
const genderMeta = {
  m: { color: '#4A7FA5', label: 'der' },
  f: { color: '#A5607A', label: 'die' },
  n: { color: '#4E8C6E', label: 'das' },
  default: { color: '#A89F94', label: '—' }
};

export default function WordList({
  searchTerm, setSearchTerm, listBook, setListBook, listLesson, setListLesson,
  books, getLessons, filteredList, handleEditClick, handleDelete, sortBy, setSortBy
}) {
  return (
    <div style={{
      width: '100%',
      ...
      <div style={{ position: 'relative', marginBottom: '20px' }}>
        <span style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: 'var(--ink-faint)', fontSize: '15px' }}>⌕</span>
        <input
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
        {[
          { val: listBook, set: setListBook, opts: books, label: b => b === 'ყველა' ? 'ყველა წიგნი' : b },
          { val: listLesson, set: setListLesson, opts: getLessons(listBook), label: l => l === 'ყველა' ? 'ყველა გაკვეთილი' : `გაკვეთილი ${l}` },
        ].map((sel, i) => (
          <select
            key={i}
            value={sel.val}
          ...
          >
            {sel.opts.map(o => <option key={o} value={o}>{sel.label(o)}</option>)}
          </select>
        ))}
      </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1, minWidth: 0 }}>
                  <div style={{
                    ...
                    background: meta.color, marginTop: '2px'
                  }} />
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '15px', color: 'var(--ink)', letterSpacing: '-0.01em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {w.de}
                    </div>
                ...
                <div style={{ display: 'flex', gap: '6px', opacity: 0, transition: 'opacity 0.15s ease', flexShrink: 0 }} className="group-hover:opacity-100">
                  <button
                    ...
                    style={{
                      ...
                      background: 'var(--surface-raised)', border: '1px solid var(--cream-border)',
